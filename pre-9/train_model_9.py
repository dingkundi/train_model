...
# pre_data = pd.read_excel('pre.xlsx')
# minmax = MinMaxScaler(feature_range=(1,2))
# data['A'] = minmax.fit_transform(np.array(data['A']).reshape(-1,1))
# data['B'] = minmax.fit_transform(np.array(data['B']).reshape(-1,1))
# data['C'] = minmax.fit_transform(np.array(data['C']).reshape(-1,1))
# pre_data['B'] = minmax.fit_transform(np.array(pre_data['B']).reshape(-1,1))
# pre_data['C'] = minmax.fit_transform(np.array(pre_data['C']).reshape(-1,1))
...
dtrain = xgb.DMatrix(pd.DataFrame(x_resampled,columns=['A','B','C','H']), y_resampled)
